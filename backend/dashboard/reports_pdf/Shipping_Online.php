 <?php
	...
	$codigoHTML='
		...
		<title>'.$company['cname'].' | '.$enviosonline.'</title>
		<style type="text/css"> 
			<!-- 
			.estilo1 { 
		...
		<main>
			  <table width="100%">
				  <tr>
					<td colspan="7" class="estilo4"><CENTER><strong> <h1>'.$enviosonline.'</h1></strong></CENTER></td>
				  </tr>
				...
				$sql=mysql_query("SELECT * FROM courier_online WHERE date   BETWEEN '$desde' AND '$hasta'");
				while($express=mysql_fetch_array($sql)){
				$codigoHTML.='	
					<tr>
						<td class="estilo3">'.$express['cons_no'].'</td>
						<td class="estilo3">'.$express['city1'].'</td>
	...
	$dompdf->render();;
	$dompdf->stream("Shipping_Online.pdf");
