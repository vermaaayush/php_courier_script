<?php
	...
	$codigoHTML='
		...
		<title>'.$company['cname'].' | '.$enviospagodebito.'</title>
		<style type="text/css"> 
			<!-- 
			.estilo1 { 
		...
		<main>
			  <table width="100%">
				  <tr>
					<td colspan="7" class="estilo4"><CENTER><strong> <h1>'.$enviospagodebito.'</h1></strong></CENTER></td>
				  </tr>
				...
				$sql=mysql_query("SELECT * FROM courier WHERE  book_mode = 'Debit_card'  AND officename='".$_SESSION["user_type"]."' AND book_date   BETWEEN '$desde' AND '$hasta'");
	...
	$dompdf->render();;
	$dompdf->stream("Debit_payment_card_shipments.pdf");
?>